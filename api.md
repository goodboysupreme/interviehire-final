...
- **2026-06-26** — POST /api/jobs/applicants/{applicant_id}/schedule + public reschedule (`/api/public/confirm`, `/api/public/reschedule`): NO schema change, but invite **delivery** fixed/changed. SMTP is now skipped on Railway (env `RAILWAY_ENVIRONMENT`) and given a 10s connect timeout elsewhere — it was hanging ~2 min on Railway's blocked SMTP port and freezing the whole request (stuck "Scheduling…"). Google Calendar is now the primary invite channel: `create_calendar_event`/`update_calendar_event` use `sendUpdates='all'` (was `'none'`) and the event description embeds the interview-room + reschedule links. Requires GOOGLE_CLIENT_ID/SECRET + a connected account (or GOOGLE_REFRESH_TOKEN); otherwise calendar stays in simulation and no invite is sent (scheduling still succeeds + provisions the session via sync_applicant_to_ai).
