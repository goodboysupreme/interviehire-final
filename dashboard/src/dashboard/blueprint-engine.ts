...
import type { Job, FunctionalParameters, ScreeningBlueprint } from '../types/models';

// The persisted/shared shapes (FunctionalParameters, ScreeningBlueprint) are the
// looser on-the-wire forms of the engine's FunctionalBlueprint / ScreeningBlueprintShape.
// Public boundary fns accept either so existing callers keep compiling.
type AnyFunctionalBlueprint = FunctionalBlueprint | FunctionalParameters | undefined;
type AnyScreeningBlueprint = ScreeningBlueprintShape | ScreeningBlueprint;

// ── Internal data model (the engine's contract-aligned shapes) ──────────────
// These are richer than the shared JobQuestion; the studio authors against them
// and serializes down to the backend contract at the boundary.
...
export interface Rubric {
  requiredPoints: RubricPoint[];
  secondaryPoints: RubricPoint[];
  excellentAnswerSignals: RubricPoint[];
  redFlags: RedFlagPoint[];
}
...
export interface FunctionalBlueprint {
  topics: Topic[];
  interviewStructure?: InterviewStructure;
  suggestedTopics?: TopicSuggestion[];
}
export interface ScreeningBlueprintShape {
  questions: QuestionBlueprint[];
}
...
const oneOf = <T extends string>(v: any, allowed: readonly T[], fallback: T): T => (allowed.includes(v) ? v as T : fallback);
const arr = <T = any>(v: any): T[] => (Array.isArray(v) ? v : []);
const clampWeight = (w: any): number => Math.max(1, Math.min(3, Math.round(Number(w) || 2)));
const snakeId = (s: any, fallback: string): string =>
  (clean(s) || fallback).toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 40) || fallback;
const dedupeReqs = (list: any): string[] => {
...
// ── Factories ──────────────────────────────────────────────────────────────
export function createRubricPoint(description: any = '', weight: any = 2, keywords: any = [], opts: AnyObj = {}): RubricPoint {
  const point: RubricPoint = { id: snakeId(description, uid('pt')), description: clean(description), keywords: arr(keywords).map((k) => clean(k)).filter(Boolean), weight: clampWeight(weight) };
...
export function createRedFlag(description: any = '', severity: any = 'medium'): RedFlagPoint {
  return { id: snakeId(description, uid('rf')), description: clean(description), severity: oneOf(severity, SEVERITY_LEVELS, 'medium') };
}

export function createQuestionBlueprint(overrides: AnyObj = {}): QuestionBlueprint {
  const difficulty = oneOf<ContractDifficulty>(overrides.difficulty, CONTRACT_DIFFICULTY as ContractDifficulty[], 'Medium');
  return {
    ...
    rubric: {
      requiredPoints: arr(overrides.rubric?.requiredPoints).filter((p: any) => p && typeof p === 'object').map((p: any) => createRubricPoint(p.description, p.weight, p.keywords, p)),
      secondaryPoints: arr(overrides.rubric?.secondaryPoints).filter((p: any) => p && typeof p === 'object').map((p: any) => createRubricPoint(p.description, p.weight ?? 1, p.keywords, p)),
      ...
      excellentAnswerSignals: arr(overrides.rubric?.excellentAnswerSignals)
        .map((s: any) => (typeof s === 'string' ? createRubricPoint(s, 1, []) : (s && typeof s === 'object' ? createRubricPoint(s.description, s.weight ?? 1, s.keywords, s) : null)))
        .filter(Boolean) as RubricPoint[],
      redFlags: arr(overrides.rubric?.redFlags).map((f: any) => createRedFlag(f.description, f.severity)),
    },
    ...
    listenFor: arr(overrides.listenFor).map((s: any) => clean(s)).filter(Boolean),
...
export function createTopic(overrides: AnyObj = {}): Topic {
  return {
...
export function createTopicSuggestion(overrides: AnyObj = {}): TopicSuggestion {
  return {
...
export function normalizeInterviewStructure(s: any): InterviewStructure {
...
export function rubricStrength(qb: QuestionBlueprint): 'ready' | 'light' | 'missing' {
  const r = qb.rubric || ({} as Rubric);
...
function pointIssue(p: RubricPoint): string | null {
...
function flagIssue(f: RedFlagPoint, qb: QuestionBlueprint): string | null {
...
function modelAnswerIssues(qb: QuestionBlueprint): string[] {
  const issues: string[] = [];
...
export function critiqueRubric(qb: QuestionBlueprint): RubricIssue[] {
  const issues: RubricIssue[] = [];
  const r = qb.rubric || ({} as Rubric);
  arr<RubricPoint>(r.requiredPoints).forEach((p) => { const m = pointIssue(p); if (m) issues.push({ level: 'warn', kind: 'point', message: m }); });
  ...
  const reqPts = arr<RubricPoint>(r.requiredPoints).filter((p) => clean(p.description));
  ...
  for (let i = 0; i < reqPts.length; i += 1) {
    for (let j = i + 1; j < reqPts.length; j += 1) {
      const a = new Set(arr<string>(reqPts[i].keywords).map((k) => k.toLowerCase()).filter(Boolean));
      const b = [...new Set(arr<string>(reqPts[j].keywords).map((k) => k.toLowerCase()).filter(Boolean))];
  ...
  const flags = arr<RedFlagPoint>(r.redFlags);
...
export function critiqueBlueprint(functionalBlueprint: FunctionalBlueprint) {
  ...
  let pointTotal = 0;
  entries.forEach(({ q }) => arr<RubricPoint>(q.rubric?.requiredPoints).forEach((p) => {
    pointTotal += 1;
    new Set(arr<string>(p.keywords).map((k) => k.toLowerCase()).filter(Boolean)).forEach((k) => kwCount.set(k, (kwCount.get(k) || 0) + 1));
  }));
  const overUsed = new Set<string>(pointTotal >= 4 ? [...kwCount.entries()].filter(([, c]) => c / pointTotal > 0.5).map(([k]) => k) : []);
  if (overUsed.size) {
    entries.forEach((entry) => {
      const generic = new Set<string>();
      arr<RubricPoint>(entry.q.rubric?.requiredPoints).forEach((p) => arr<string>(p.keywords).forEach((k) => { if (overUsed.has(k.toLowerCase())) generic.add(k.toLowerCase()); }));
...
export function leakageRisk(qb: QuestionBlueprint): { risk: string; reason: string } {
...
export function migrateLegacyQuestions(legacyQuestions: any): FunctionalBlueprint {
  const groups = new Map<string, QuestionBlueprint[]>();
  arr(legacyQuestions).forEach((q: any) => {
    const qType = (LEGACY_TYPE_TO_QUESTION_TYPE as Record<string, string>)[q.type] || 'general';
    const difficulty = (LEGACY_TO_CONTRACT_DIFFICULTY as Record<string, string>)[q.difficulty] || 'Medium';
...
export function ensureFunctionalBlueprint(job: Job): FunctionalBlueprint {
  if (job.functionalParameters && Array.isArray(job.functionalParameters.topics)) return job.functionalParameters as unknown as FunctionalBlueprint;
  return migrateLegacyQuestions(job.questions);
}

// ── Generation prompts ──────────────────────────────────────────────────────
function jdContext(job: Job): string {
  const must = arr<string>(job.resumeCriteria?.mustHave);
  const good = arr<string>(job.resumeCriteria?.goodToHave);
  return [
...
function buildOutlineMessages(job: Job, opts: AnyObj = {}): ChatMessage[] {
  const seed = arr(opts.topicSeed).map((t: any) => clean(t.name)).filter(Boolean);
...
function buildEnrichMessages(job: Job, q: QuestionBlueprint, topicName: string): ChatMessage[] {
  const tier = (ENRICH_BY_DIFFICULTY as Record<string, string>)[q.difficulty] || ENRICH_BY_DIFFICULTY.Medium;
...
function buildScreeningMessages(job: Job, opts: AnyObj = {}): ChatMessage[] {
  const count = opts.count || 4;
  const categories = arr(job.screeningParams).map((c: any) => c.category).filter(Boolean);
...
async function callJson(messages: ChatMessage[]): Promise<any> {
  const raw = await (callDeepSeekAPI as (...args: any[]) => Promise<any>)(messages, true);
  return parseAIJson(sanitizeJSONResponse(raw));
}

export async function generateFunctionalOutline(job: Job, opts: AnyObj = {}): Promise<FunctionalBlueprint> {
...
export async function enrichQuestionRubric(job: Job, q: QuestionBlueprint, topicName: string) {
  const parsed = await callJson(buildEnrichMessages(job, q, topicName));
  return normalizeRubricPayload(parsed);
}

export function normalizeRubricPayload(parsed: any) {
...
export async function generateScreeningQuestions(job: Job, opts: AnyObj = {}): Promise<ScreeningBlueprintShape> {
...
function buildTopicSuggestionMessages(job: Job, requirements: string[] = []): ChatMessage[] {
...
export async function suggestTopics(job: Job, requirements: string[] = []): Promise<TopicSuggestion[]> {
  const parsed = await callJson(buildTopicSuggestionMessages(job, requirements));
  const out = arr(parsed?.suggestedTopics)
    .map((t: any) => createTopicSuggestion(t))
...
export function localTopicSuggestions(job: Job, requirements: string[] = []): TopicSuggestion[] {
...
function buildRequirementMessages(job: Job): ChatMessage[] {
...
function localRequirements(job: Job): string[] {
...
export async function analyzeRequirements(job: Job): Promise<string[]> {
  ...
  try {
    ...
    const llm = arr(parsed?.requirements).map((r: any) => clean(r)).filter(Boolean);
...
function buildGapMessages(job: Job, requirement: string): ChatMessage[] {
...
export async function generateGapQuestion(job: Job, requirement: string): Promise<QuestionBlueprint> {
...
function buildScenarioMessages(job: Job, qb: QuestionBlueprint): ChatMessage[] {
...
export async function generateScenarioVariant(job: Job, qb: QuestionBlueprint): Promise<QuestionBlueprint> {
...
function buildDifficultyMessages(job: Job, qb: QuestionBlueprint, target: string): ChatMessage[] {
  const competency = clean(qb.competency) || clean(qb.prompt);
...
export async function generateDifficultyVariant(job: Job, qb: QuestionBlueprint, target: string): Promise<QuestionBlueprint> {
...
export function localDifficultyVariant(qb: QuestionBlueprint, target: string): QuestionBlueprint {
...
export function normalizeFunctionalBlueprint(parsed: any): FunctionalBlueprint {
  const topics = arr(parsed?.topics).length ? parsed.topics : arr(parsed);
  const out: FunctionalBlueprint = { topics: topics.map((t: any) => createTopic({
    name: t.name || t.topic,
    type: t.type,
    difficulty: t.difficulty,
    whyItMatters: t.whyItMatters,
    segue: t.segue,
    questions: arr(t.questions).map((q: any) => createQuestionBlueprint(q)),
  })).filter((t: Topic) => t.questions.length) };
  if (parsed?.interviewStructure) out.interviewStructure = normalizeInterviewStructure(parsed.interviewStructure);
  if (arr(parsed?.suggestedTopics).length) out.suggestedTopics = parsed.suggestedTopics.map((s: any) => createTopicSuggestion(s));
  return out;
}

export function normalizeScreeningBlueprint(parsed: any): ScreeningBlueprintShape {
  const list = arr(parsed?.questions).length ? parsed.questions : arr(parsed);
  return { questions: list.map((q: any) => createQuestionBlueprint({
    ...q,
    questionType: 'hr_screening',
    difficulty: 'Easy',
  })).filter((q: QuestionBlueprint) => q.prompt) };
}
...
const normReq = (s: any): string => clean(s).toLowerCase().replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
export function computeCoverage(job: Job, functionalBlueprint: FunctionalBlueprint) {
  ...
  const mustHave = arr<string>(job.resumeCriteria?.mustHave);
  let must: string[] = mustHave;
  if (!must.length) {
    const seen = new Map<string, string>();
    questions.forEach((q) => { const t = clean(q.targetRequirement); if (t) seen.set(normReq(t), t); });
    must = [...seen.values()];
  }
  const hayOf = (q: QuestionBlueprint) => `${q.competency} ${q.prompt} ${q.rubric.requiredPoints.map((p) => `${p.description} ${p.keywords.join(' ')}`).join(' ')}`.toLowerCase();
  return must.map((req) => {
...
function matchRequirement(haystack: any, requirements: string[]): string {
...
export function pinBlueprintToRequirements(job: Job, functionalBlueprintArg: AnyFunctionalBlueprint, requirements: string[]): FunctionalBlueprint {
  const functionalBlueprint = functionalBlueprintArg as FunctionalBlueprint;
  const reqs = dedupeReqs(requirements);
  if (!reqs.length || !functionalBlueprint || !Array.isArray(functionalBlueprint.topics)) return functionalBlueprint;
  const byNorm = new Map<string, string>(reqs.map((r) => [normReq(r), r]));
...
export function mergeBlueprintPreservingEdits(existingFbArg: AnyFunctionalBlueprint, freshFbArg: AnyFunctionalBlueprint): FunctionalBlueprint {
  const existingFb = existingFbArg as FunctionalBlueprint;
  const freshFb = freshFbArg as FunctionalBlueprint;
  const merged: FunctionalBlueprint = freshFb && Array.isArray(freshFb.topics) ? freshFb : { topics: [] };
  ...
  const edited: { q: QuestionBlueprint; topicName: string }[] = [];
...
export function computeGenerationPlan(job: Job) {
...
export function computeCalibration(functionalBlueprint: FunctionalBlueprint) {
  const questions = (functionalBlueprint.topics || []).flatMap((t) => t.questions);
  const totalMinutes = questions.reduce((a, q) => a + (q.estimatedMinutes || (MINUTES_BY_DIFFICULTY as Record<string, number>)[q.difficulty] || 4), 0);
  const difficultyMix = CONTRACT_DIFFICULTY.reduce((m: Record<string, number>, d) => { m[d] = questions.filter((q) => q.difficulty === d).length; return m; }, {});
...
export function topicMinutes(topic: Topic | undefined): number {
  return arr<QuestionBlueprint>(topic?.questions).reduce((a, q) => a + (q.estimatedMinutes || (MINUTES_BY_DIFFICULTY as Record<string, number>)[q.difficulty] || 4), 0);
}
...
export function autofillOutlineNotes(functionalBlueprint: AnyFunctionalBlueprint, job: Job = {}): FunctionalBlueprint {
  const fb = functionalBlueprint as FunctionalBlueprint;
  ...
  topics.forEach((t, ti) => {
    ...
    t.questions.forEach((q) => {
      if (!arr(q.listenFor).length) {
        const pts = arr<RubricPoint>(q.rubric?.requiredPoints).map((p) => clean(p.description)).filter(Boolean).slice(0, 3);
...
export function runSheetMarkdown(job: Job, functionalBlueprint: FunctionalBlueprint): string {
...
export function bandTier(experienceBand: any): 'junior' | 'mid' | 'senior' {
...
export function computeBandFit(job: Job, functionalBlueprint: FunctionalBlueprint) {
  const tier = bandTier(job.experienceBand);
  const profile = TIER_PROFILE[tier];
  const questions = (functionalBlueprint.topics || []).flatMap((t) => t.questions);
  const n = questions.length;
  const actualCount = CONTRACT_DIFFICULTY.reduce((m: Record<string, number>, d) => { m[d] = questions.filter((q) => q.difficulty === d).length; return m; }, {});
  const targetCount = CONTRACT_DIFFICULTY.reduce((m: Record<string, number>, d) => { m[d] = Math.round(((profile.difficulty as Record<string, number>)[d] || 0) * n); return m; }, {});
  const typeCounts = questions.reduce((m: Record<string, number>, q) => { m[q.questionType] = (m[q.questionType] || 0) + 1; return m; }, {});

  const recommendations: { level: string; message: string }[] = [];
...
function serializeRubricPoint(p: RubricPoint): AnyObj {
  const out: AnyObj = { id: p.id, description: p.description, keywords: p.keywords, weight: p.weight };
...
export function toAiEvaluationGuidance(qb: QuestionBlueprint): string {
  return JSON.stringify({
    ...
    rubric: {
      ...
      redFlags: qb.rubric.redFlags.map((f: RedFlagPoint) => ({ id: f.id, description: f.description, severity: f.severity })),
    },
  });
}
...
export function toFunctionalParameters(functionalBlueprintArg: AnyFunctionalBlueprint): AnyObj {
  const functionalBlueprint = functionalBlueprintArg as FunctionalBlueprint;
  ...
  if (arr(functionalBlueprint.suggestedTopics).length) {
    out.suggestedTopics = functionalBlueprint.suggestedTopics!.map((s: TopicSuggestion) => ({
...
export function toScreeningQuestions(screeningBlueprint: AnyScreeningBlueprint): string[] {
  const sb = screeningBlueprint as ScreeningBlueprintShape;
  return (sb.questions || []).map((q) => q.prompt);
}
...
function lq(prompt: string, questionType: string, difficulty: string, competency: string, modelAnswer: string, required?: any[], redFlags?: any[]): QuestionBlueprint {
  return createQuestionBlueprint({
    prompt, questionType, difficulty, competency, modelAnswer,
    rubric: {
      requiredPoints: (required || []).map(([description, keywords, weight]: [string, string[], number]) => ({ description, keywords, weight })),
      redFlags: (redFlags || []).map(([description, severity]: [string, string]) => ({ description, severity })),
    },
  });
}

function roleArchetype(job: Job): string {
...
export function localFunctionalBlueprint(job: Job): FunctionalBlueprint {
  const a = roleArchetype(job);
  const T = (name: string, type: string, difficulty: string, questions: QuestionBlueprint[]) => createTopic({ name, type, difficulty, questions });
  if (a === 'engineering') {
    return { topics: [
...
export function localScreeningQuestions(job: Job): ScreeningBlueprintShape {
...
function requirementKeywords(requirement: any): string[] {
  return clean(requirement).toLowerCase().split(/[^a-z0-9+#.]+/).filter((w) => w.length > 3 && !REQ_STOPWORDS.includes(w)).slice(0, 4);
}

export function localGapQuestion(job: Job, requirement: any): QuestionBlueprint {
...
export function localScenarioVariant(qb: QuestionBlueprint): QuestionBlueprint {
