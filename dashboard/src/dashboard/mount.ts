...
function initMountBindings() {
  ...
  document.querySelectorAll('.sidebar-nav .nav-item').forEach(item => {
    item.addEventListener('click', (e) => {
      const tabId = item.getAttribute('data-tab') as string;
  ...
  document.querySelectorAll('.sub-nav li').forEach(subItem => {
    subItem.addEventListener('click', (e) => {
      e.stopPropagation();
      const subtabId = subItem.getAttribute('data-subtab') as string;
  ...
  if (btnSaveDrawerJd) {
    btnSaveDrawerJd.addEventListener('click', () => {
      ...
      const descriptionText = (document.getElementById('drawer-jd-text') as HTMLTextAreaElement).value.trim();
      if (jobId) {
        const job = AppState.jobs.find((j: Job) => j.id === jobId);
        if (job) {
          ...
          if (AppState.activeJobId === jobId) {
            const jdRawDescTextarea = document.getElementById('jd-raw-description') as HTMLTextAreaElement | null;
  ...
  if (btnEnhanceDrawerJd) {
    btnEnhanceDrawerJd.addEventListener('click', async () => {
      const drawer = document.getElementById('drawer-view-jd');
      const textarea = document.getElementById('drawer-jd-text') as HTMLTextAreaElement | null;
      ...
      const originalLabel = btnEnhanceDrawerJd.textContent as string;
      (btnEnhanceDrawerJd as HTMLButtonElement).disabled = true;
      ...
      try {
        ...
        (btnEnhanceDrawerJd as HTMLButtonElement).disabled = false;
        ...
        if (accepted !== null && textarea) {
          textarea.value = accepted as string;
  ...
  if (btnGenerateFromDrawer) {
    btnGenerateFromDrawer.addEventListener('click', () => {
      const drawer = document.getElementById('drawer-view-jd')!;
      const jobId = drawer.getAttribute('data-current-job-id') as string;
      const descriptionText = (document.getElementById('drawer-jd-text') as HTMLTextAreaElement).value.trim();
      ...
      requestAnimationFrame(() => {
        const questionsTab = document.querySelector('.jd-tab[data-jd-tab="questions"]') as HTMLElement | null;
  ...
  window.handleJobKebab = function (jobId, action) {
    ...
    switch (action) {
      ...
      case 'settings':
        navigateToJobDetail(jobId);
        setTimeout(() => {
          const qTab = document.querySelector('.jd-tab[data-jd-tab="questions"]') as HTMLElement | null;
          if (qTab) qTab.click();
        }, 100);
        break;
      case 'archive':
      ...
      case 'delete': {
        ...
        const removedCandidates = AppState.candidates.filter((c: Candidate) => isBackend
          ? c.jobId === job.id
          : (c.jobApplied === job.roleName || c.jobApplied === job.cardName));
        AppState.jobs.splice(idx, 1);
        AppState.candidates = AppState.candidates.filter((c: Candidate) => isBackend
          ? c.jobId !== job.id
          : (c.jobApplied !== job.roleName && c.jobApplied !== job.cardName));
        saveStateToLocalStorage();
        const restoreJob = () => {
  ...
  let editJobModalTags: string[] = [];
  let editJobModalJobId: string | null = null;
  ...
  function openEditJobModal(jobId: string) {
    ...
    (document.getElementById('modal-edit-job-name') as HTMLInputElement).value = job.cardName || job.roleName || '';
    (document.getElementById('modal-edit-job-id') as HTMLInputElement).value = job.customJobId && job.customJobId !== '-' ? job.customJobId : '';
    renderEditJobTags();
    modal.style.display = '';
    setTimeout(() => (document.getElementById('modal-edit-job-name') as HTMLInputElement).focus(), 50);
    soundEngine.playChime([392.00, 523.25], 0.12, 0.1);
  }

  function closeEditJobModal() {
    (document.getElementById('modal-edit-job') as HTMLElement).style.display = 'none';
  ...
  function renderEditJobTags() {
    ...
    list.innerHTML = editJobModalTags.map((tag: string, i: number) =>
    ...
    list.querySelectorAll('.modal-tag-remove').forEach(btn => {
      btn.addEventListener('click', () => {
        editJobModalTags.splice(parseInt((btn as HTMLElement).dataset.idx as string), 1);
        renderEditJobTags();
      });
    });
  }
  ...
  if (modalEditJob) {
    modalEditJob.addEventListener('click', (e) => {
      if ((e.target as HTMLElement).id === 'modal-edit-job') closeEditJobModal();
    });
  }
  ...
  if (modalEditJobSave) {
    modalEditJobSave.addEventListener('click', () => {
      const job: Job | undefined = AppState.jobs.find((j: Job) => j.id === editJobModalJobId);
      if (!job) return;
      const nameVal = (document.getElementById('modal-edit-job-name') as HTMLInputElement).value.trim();
      if (!nameVal) {
        showPremiumToast('Job name is required.', 'error');
        return;
      }
      job.cardName = nameVal;
      const idVal = (document.getElementById('modal-edit-job-id') as HTMLInputElement).value.trim();
  ...
  document.querySelectorAll('.filter-options button[data-filter]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      btn.classList.add('active');
      AppState.jobsFilter = btn.getAttribute('data-filter') as string;
      soundEngine.playClick();

      const isBoard = (document.getElementById('btn-view-board') as HTMLElement).classList.contains('active');
      if (isBoard) {
        renderKanbanBoard();
      } else {
        renderJobCards();
  ...
  document.querySelectorAll('#team-status-tabs button').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      btn.classList.add('active');
      AppState.teamFilter = btn.getAttribute('data-team-filter') as string;
      soundEngine.playClick();
      renderTeamTable();
    });
  });
  ...
  document.querySelectorAll('.table-tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      btn.classList.add('active');
      AppState.analyticsSubtab = btn.getAttribute('data-table') as string;
      soundEngine.playClick();
  ...
  globalSearchInput.addEventListener('input', (e) => {
    AppState.globalSearch = (e.target as HTMLInputElement).value;
    if (AppState.activeTab === 'jobs') {
      ...
      AppState.tableSearch = (e.target as HTMLInputElement).value;
      renderAnalyticsTable();
    ...
    analyticsFilterBtn.addEventListener('click', (e) => {
      ...
      if (AppState.analyticsSubtab === 'jobs-data') {
        ...
        dd.innerHTML = `
          ...
          <div class="afd-items">${statuses.map(s => `<label class="afd-item"><input type="checkbox" value="${s}" ${(AppState.analyticsJobStatusFilter as string[] | undefined)?.includes(s) ? 'checked' : ''} /><span>${s}</span></label>`).join('')}</div>
        ...
        dd.querySelector('.afd-apply')!.addEventListener('click', () => {
          (AppState as AppStateType).analyticsJobStatusFilter = [...dd.querySelectorAll('input:checked')].map(c => (c as HTMLInputElement).value);
          renderAnalyticsTable();
          dd.remove();
        });
        dd.querySelector('.afd-clear')!.addEventListener('click', () => {
          (AppState as AppStateType).analyticsJobStatusFilter = [];
          renderAnalyticsTable();
          dd.remove();
        });
      } else {
        ...
        dd.innerHTML = `
          ...
          <div class="afd-items">${stages.map(s => `<label class="afd-item"><input type="checkbox" value="${s}" ${(AppState.analyticsCandStageFilter as string[] | undefined)?.includes(s) ? 'checked' : ''} /><span>${s}</span></label>`).join('')}</div>
          <div class="afd-footer"><button class="afd-clear">Clear</button><button class="afd-apply">Apply</button></div>`;
        dd.querySelector('.afd-apply')!.addEventListener('click', () => {
          (AppState as AppStateType).analyticsCandStageFilter = [...dd.querySelectorAll('input:checked')].map(c => (c as HTMLInputElement).value);
          renderAnalyticsTable();
          dd.remove();
        });
        dd.querySelector('.afd-clear')!.addEventListener('click', () => {
          (AppState as AppStateType).analyticsCandStageFilter = [];
          renderAnalyticsTable();
          dd.remove();
        });
      }
      (analyticsFilterBtn.parentElement as HTMLElement).style.position = 'relative';
      analyticsFilterBtn.parentElement!.appendChild(dd);
      const close = (ev: Event) => { if (!dd.contains(ev.target as Node) && ev.target !== analyticsFilterBtn) { dd.remove(); document.removeEventListener('click', close); } };
  ...
  if (createJobForm) {
    createJobForm.addEventListener('submit', async (e) => {
      ...
      const cardName = (document.getElementById('job-title-input') as HTMLInputElement).value;
      const roleName = (document.getElementById('job-role-input') as HTMLInputElement).value;
      const expBand = (document.getElementById('job-experience-input') as HTMLInputElement).value;
      let customId = (document.getElementById('job-custom-id') as HTMLInputElement).value;
      const description = (document.getElementById('job-description-input') as HTMLTextAreaElement).value.trim();
      ...
      const addResume = (document.getElementById('chk-resume') as HTMLInputElement).checked;
      const addScreening = (document.getElementById('chk-screening') as HTMLInputElement).checked;
      const addFunctional = (document.getElementById('chk-functional') as HTMLInputElement).checked;
  ...
  const careerThemeSelect = document.getElementById('career-theme') as HTMLSelectElement | null;
  ...
  if (careerThemeSelect) {
    careerThemeSelect.addEventListener('change', (e) => {
      const shouldBeLight = (e.target as HTMLSelectElement).value === 'light';
...
{"roleName":"exact job title","cardName":"job title + brief context","experienceBand":"one of: ${EXPERIENCE_BANDS_PROMPT}","description":"clean 2-3 sentence professional job description"}`;
      ...
      try {
        ...
        if (newJob._backend) {
          try { await apiPatchJobParameters(newJob.id as string, newJob); }
