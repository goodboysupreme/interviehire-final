...
import { AppState as AppStateRaw } from './state';
import { activeCandidateSubTabs } from './vetting-data';
import { getDataSource, isApiMode, apiScheduleCandidate, apiUpdateApplicant, apiMoveApplicantStage, ensureBackendApplicantId, apiUploadResumes } from './api';
import type { AppState as AppStateType, Candidate, Job } from '../types/models';

// Typed view over the shared runtime store (same object reference). The models
// AppState interface gives `candidates: Candidate[]`, `jobs: Job[]` and a
// permissive index signature for the dynamic UI keys this module reads/writes.
const AppState = AppStateRaw as unknown as AppStateType;

function getCandidateSubtab(c: Candidate) {
...
function getOrInitActiveSubtab(stageKey: string, cands: Candidate[]) {
...
function buildSubtabsBarHTML(stageKey: string, counts: Record<string, number>, activeSubtab: string) {
  const tabs = [
    {
      id: 'window-missed',
      label: 'Window Missed',
      ...
      screeningList.innerHTML = `
        ...
        return `
                  <tr data-candidate-id="${c.id}">
                    ...
                    <td><span class="source-badge">${sourceIcon} ${sourceLabel(c.entryMethod)}</span></td>
                    <td>${c.attemptedAt || '—'}</td>
...
function bindAddApplicantsPanel(job: Job, paneKey: string) {
  ...
  if (addBtn) {
    addBtn.addEventListener('click', (e) => {
      e.preventDefault();
      import('./sourcing').then(({ navigateToSourcing }) => {
