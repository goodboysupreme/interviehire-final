...
export function buildGreeting() {
  const name = ((globalThis as any).IH_USER_NAME || '').trim();
  const h = new Date().getHours();
  const part = h < 12 ? 'Good morning' : h < 18 ? 'Good afternoon' : 'Good evening';
  const icon = h < 12 ? '🌤️' : h < 18 ? '☀️' : '🌙';
  return name ? `${part}, ${name} ${icon}` : `${part} ${icon}`;
}
try { (globalThis as any).__ihBuildGreeting = buildGreeting; } catch {}
...
function navigateToTab(tabId: string) {
  ...
  if (tabId === 'jobs') {
    ...
    renderTalentFinderPane(null as any);
...
function navigateToCreateJob() {
  ...
  const fileInput = document.getElementById('jd-file-input') as HTMLInputElement | null;
...
let ariaChatHistory: { role: string; content: string }[] = [];
...
function navigateToAriaChat() {
  ...
  const chatInput = document.getElementById('aria-chat-input') as HTMLInputElement | null;
  if (chatInput) { chatInput.value = ''; chatInput.disabled = false; }
  const sendBtn = document.getElementById('btn-aria-send') as HTMLButtonElement | null;
...
function appendAriaMessage(text: string, sender: string) {
...
async function sendAriaMessage(text: string) {
  if (!text.trim()) return;
  const input = document.getElementById('aria-chat-input') as HTMLInputElement;
  const sendBtn = document.getElementById('btn-aria-send') as HTMLButtonElement;
  input.value = '';
  input.disabled = true;
...
{"ready":false,"message":"your warm 1-2 sentence follow-up question"}`;
  ...
  try {
    const response = await (callDeepSeekAPI as any)([
    ...
    if (parsed.ready) {
      ...
      const newJob: any = isApiMode() ? await apiCreateJob(jobDraft as any) : { ...jobDraft, id: generateJobId() };
...
function navigateToSubtab(subtabId: string) {
  AppState.activeTab = 'settings';
  AppState.activeSubtab = subtabId;

  const SUBTAB_URLS: Record<string, string> = {
...
// ==========================================
function openDrawer(drawerType: string, jobId: string | null = null) {
  ...
  if (drawerType === 'job') {
    ...
    if (jobId) {
      ...
      if (job) {
        (document.getElementById('drawer-jd-text') as HTMLTextAreaElement).value = job.description || "";
...
// ==========================================
function triggerExcelExport(dataType: string) {
  soundEngine.playChime([523.25, 659.25, 783.99], 0.2, 0.08);
