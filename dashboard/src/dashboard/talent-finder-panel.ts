...
// Talent-finder-specific dynamic API shapes (distinct from the dashboard Candidate).
interface TfFitBreakdown {
...
interface TfSourceRef {
  source_name?: string;
  source_type?: string;
  source_permission_status?: string;
}
interface TfCandidate {
  id?: string;
  full_name?: string;
...
let CSV_ROWS: any[] = [];
let LAST_RESULTS: TfCandidate[] = [];
let LAST_BRIEF: TfBrief = {};
let CURRENT_JOB: Job | null = null;
...
// The DOM lookup is genuinely dynamic here; widen to a permissive element type so
// the many `.value`/`.checked`/`.style`/`.onclick` reads below type-check. Runtime
// is identical (a non-null assertion + cast both erase at build).
...
const statusPill = (s?: string) => `<span class="tf-stat ${s || 'new'}">${esc(s || 'new')}</span>`;
...
async function tfCsvUpload(file: File) {
...
export async function renderTalentFinderPane(job: Job) {
  injectStyle();
  CURRENT_JOB = job;
  ...
  $('tf-intl').onchange = (e) => { $('tf-intl-wrap').style.display = (e.target as HTMLInputElement).checked ? 'block' : 'none'; };
  $('tf-csv').onchange = async (e) => {
    ...
    } catch (err) { const n = $('tf-csvnote'); n.style.display = 'block'; n.className = 'tf-note tf-err'; n.textContent = (err as Error).message; }
  };

  await Promise.all([loadSources(), autoFill(job)]);
}

async function autoFill(job: Job) {
  try {
    const d = await request('/talent-finder/extract-brief', { method: 'POST', body: { jobRoleId: job && job.id } } as any);
    const b = d.brief || {};
    $('tf-title').value = b.title || (job && (job.roleName || job.cardName)) || '';
    $('tf-location').value = b.location || (job && (job as any).location) || '';
...
async function loadSources() {
  try {
    ...
    $('tf-sources').innerHTML = (window._tfSources as TfSource[]).map((s: TfSource) => `
      <div class="tf-src">
    ...
    $('tf-sources').innerHTML = '<span class="tf-err">Sign in to the dashboard first. ' + esc((e as Error).message) + '</span>';
  }
}

function selectedSources() {
  return ((window._tfSources || []) as TfSource[]).filter((s: TfSource) => { const c = $('tf-src-' + s.source_type); return c && c.checked; }).map((s: TfSource) => s.source_type);
}
function setSteps(active: string) {
  const order = ['searching', 'deduping', 'ranking', 'done'];
  const labels: Record<string, string> = { searching: 'Searching sources', deduping: 'Removing duplicates', ranking: 'Ranking', done: 'Done' };
  $('tf-steps').innerHTML = order.map((k) => `<span class="tf-step ${order.indexOf(k) <= order.indexOf(active) ? 'done' : ''}">${labels[k]}</span>`).join('');
}
function scoreClass(s: number | null | undefined) { return (s as number) >= 75 ? 'tf-strong' : (s as number) >= 55 ? 'tf-good' : (s as number) >= 40 ? 'tf-mid' : 'tf-weak'; }
...
async function runSearch() {
  ...
  try {
    const r = await request('/talent-finder/search', { method: 'POST', body } as any);
    setSteps('done');
    ...
    $('tf-results').innerHTML = '<span class="tf-err">Search failed: ' + esc((e as Error).message) + '</span>'; setSteps('searching');
  }
  btn.disabled = false;
}
...
async function loadResults(searchId: string) {
  ...
  $('tf-results').innerHTML = `<table class="tf-table"><thead><tr>
    ...
    <tbody>${LAST_RESULTS.map((c: TfCandidate, i: number) => { const fb = c.fit_breakdown || {}; const mh = (fb.matchedMustHaves || []).length, mt = mh + (fb.missingMustHaves || []).length;
  ...
  $('tf-results').querySelectorAll('tr.row').forEach((tr) => (tr as HTMLElement).onclick = () => openDrawer(parseInt(tr.getAttribute('data-i')!)));
}

function openDrawer(i: number) {
  const c = LAST_RESULTS[i]; const fb = c.fit_breakdown || {};
  const bars: [string, number | undefined][] = [['Must-have', fb.mustHaveScore], ['Experience', fb.experienceScore], ['Title/semantic', fb.semanticScore], ['Location', fb.locationScore], ['Good-to-have', fb.goodToHaveScore]];
  ...
  $('tf-gen').onclick = (e) => {
    const b = e.currentTarget as HTMLButtonElement; b.disabled = true; const old = b.textContent; b.textContent = '✨ Generating…';
    act('/candidates/' + c.id + '/generate-outreach', { method: 'POST', body: { brief: LAST_BRIEF } } as any,
...
async function act(path: string, opts: { method?: string; body?: any }, after?: (d: any) => void, errMsg?: string) {
  try {
    const d = await request('/talent-finder' + path, opts as any);
    if (after) after(d);
    return true;
  } catch (e) {
    toast((errMsg ? errMsg + ': ' : '') + ((e as Error).message || 'Request failed'), 'err');
    return false;
  }
}
