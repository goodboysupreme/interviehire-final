...
export function reviewJdRewrite({ title = 'AI Suggestion', original = '', suggested = '' }) {
  ...
  return new Promise((resolve) => {
    ...
    const modal = overlay.querySelector('.jd-rw-modal') as HTMLElement;
    const suggestionInput = overlay.querySelector('.jd-rw-suggestion') as HTMLTextAreaElement;
    const focusables = () => Array.from(modal.querySelectorAll('button, textarea')).filter((el) => !(el as HTMLButtonElement | HTMLTextAreaElement).disabled) as HTMLElement[];
