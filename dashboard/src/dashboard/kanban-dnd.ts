...
let activeCardPlayerId: string | null = null;
let activeCardInterval: ReturnType<typeof setInterval> | null = null;
...
function initKanbanDragAndDrop() {
  ...
  Object.entries(cols).forEach(([stage, col]) => {
    ...
    col.addEventListener('drop', async (e) => {
      ...
      const candidate = (AppState.candidates as Candidate[]).find((c: Candidate) => c.id === candidateId);
      ...
      if (candidate && candidate.status !== stage) {
        ...
        if (getDataSource() === 'api' && candidate._backend) {
          try {
            ...
            const updated = await apiMoveApplicantStage((candidate.backendId || candidate.id) as string, stage);
            if (updated) {
      ...
      columns.forEach(col => {
        ...
        label.querySelector('input')!.addEventListener('change', (e) => {
          ...
          if (isChecked) {
            ...
            AppState.visibleColumnsAnalyticsJobs = AppState.visibleColumnsAnalyticsJobs.filter((id: string) => id !== col.id);
          }
      ...
      columns.forEach(col => {
        ...
        label.querySelector('input')!.addEventListener('change', (e) => {
          ...
          if (isChecked) {
            ...
            AppState.visibleColumnsAnalyticsCandidates = AppState.visibleColumnsAnalyticsCandidates.filter((id: string) => id !== col.id);
          }
          soundEngine.playClick();
          renderAnalyticsTable();
        });
    ...
    columns.forEach(col => {
      ...
      label.querySelector('input')!.addEventListener('change', (e) => {
        ...
        if (isChecked) {
          ...
          AppState.visibleColumnsTeam = AppState.visibleColumnsTeam.filter((id: string) => id !== col.id);
        }
        soundEngine.playClick();
        renderTeamTable();
      });
...
function stopActiveCardPlayer() {
  ...
  if (activeCardPlayerId) {
    ...
    if (playBtn) {
      (playBtn.querySelector('.play-icon') as HTMLElement).style.display = 'block';
      (playBtn.querySelector('.pause-icon') as HTMLElement).style.display = 'none';
    }
    ...
    bars.forEach((b) => {
      ...
      (b as HTMLElement).style.setProperty('--wave-height', (Math.floor(Math.random() * 70 + 20)) / 100 as unknown as string);
    });
    activeCardPlayerId = null;
  }
}

function toggleCardPlayer(id: string) {
  if (activeCardPlayerId === id) {
    ...
    const playBtn = document.querySelector(`[data-play-id="${id}"]`);
    if (playBtn) {
      (playBtn.querySelector('.play-icon') as HTMLElement).style.display = 'none';
      (playBtn.querySelector('.pause-icon') as HTMLElement).style.display = 'block';
    }
