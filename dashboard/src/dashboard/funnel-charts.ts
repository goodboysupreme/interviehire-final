...
function drawFunnelSVG(job: Job, candidates: Candidate[]) {
  const svgEl = document.getElementById('jd-funnel-svg') as SVGSVGElement | null;
  if (!svgEl) return;
  ...
  const total = Math.max(pipeline.total as number, 1);
  ...
  const stageLabels: string[] = ['Total Candidates'];
  const stageCounts: number[] = [pipeline.total as number];
  if (includeResume) { stageLabels.push('Resume Analysis'); stageCounts.push(pipeline.resume || 0); }
  if (includeScreening) { stageLabels.push('Recruiter Screening'); stageCounts.push(pipeline.screening || 0); }
  if (includeFunctional) { stageLabels.push('Functional Interview'); stageCounts.push(pipeline.functional || 0); }
  ...
  const stageStatusMap: Record<string, string | null> = {
  ...
  function getBreakdownForStage(stageLabel: string): Record<string, number> {
    const status = stageStatusMap[stageLabel];
    let stageCands: Candidate[];
  ...
  function getSourceFractions(stageIdx: number) {
    ...
    const fracs: Array<{ source: string; frac: number; color: string }> = [];
  ...
  pts.slice(1, -1).forEach(p => {
    ...
    line.setAttribute('x1', (p.lx - 14) as any);
    line.setAttribute('y1', p.y as any);
    line.setAttribute('x2', (p.rx + 14) as any);
    line.setAttribute('y2', p.y as any);
  ...
  if (n > 2) {
    ...
    for (let i = 1; i <= n - 2; i++) {
      ...
      const pickFirst = (arr: Array<{ color: string }>) => (arr.length ? arr[0].color : '#888');
      ...
      rect.setAttribute('x', maxLx as any);
      rect.setAttribute('y', (bY - bandH / 2) as any);
      rect.setAttribute('width', (maxRx - maxLx) as any);
      rect.setAttribute('height', bandH as any);
  ...
  const stageItems = document.querySelectorAll('#jd-funnel-stages .jd-stage-item') as NodeListOf<HTMLElement>;
  ...
  function showTooltip(idx: number, clientX: number, clientY: number) {
    if (activeSegIdx === idx) {
  ...
  svgEl.addEventListener('mousemove', function (e: MouseEvent) {
    const target = e.target as Element | null;
    const g = target && target.closest ? target.closest('g[data-stage-idx]') : null;
    if (!g && target && target.tagName === 'path') {
      const parent = target.parentElement;
      if (parent && parent.tagName.toLowerCase() === 'g' && parent.hasAttribute('data-stage-idx')) {
        showTooltip(parseInt(parent.getAttribute('data-stage-idx') as string), e.clientX, e.clientY);
        return;
      }
    }
    if (g) {
      showTooltip(parseInt(g.getAttribute('data-stage-idx') as string), e.clientX, e.clientY);
    } else {
      hideTooltip();
    }
  });
...
function drawScoreDistributionSVG(job: Job, candidates: Candidate[]) {
  const svgEl = document.getElementById('jd-score-svg') as SVGSVGElement | null;
  if (!svgEl) return;
  ...
  candidates.forEach(c => {
    const s = parseFloat(c.score as any);
    if (s < 20) counts[0]++;
