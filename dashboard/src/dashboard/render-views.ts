...
import type { AppState as AppStateModel, Job, Candidate, TeamMember } from '../types/models';
...
function renderJobCards() {
  ...
  container.innerHTML = '';
  const filteredJobs = (AppState.jobs as Job[]).filter((job: Job) => {
    // Filter status tabs
  ...
  filteredJobs.forEach(job => {
    ...
    const createdBy = job.createdBy || (globalThis as any).IH_USER_NAME || 'You';
    ...
    card.addEventListener('click', (e) => {
      if ((e.target as HTMLElement).closest('.job-card-header-actions, .card-flow-cta, .author-link-doc')) return;
...
function updateJobsCounters() {
  ...
  document.querySelector('.count-all')!.textContent = allCount as any;
  document.querySelector('.count-published')!.textContent = publishedCount as any;
  document.querySelector('.count-draft')!.textContent = draftCount as any;
  document.querySelector('.count-archived')!.textContent = archivedCount as any;
}
...
function renderAnalyticsTable() {
  ...
  if (AppState.analyticsSubtab === 'jobs-data') {
    ...
    if (AppState.analyticsJobStatusFilter?.length > 0) {
      list = list.filter(j => (AppState.analyticsJobStatusFilter as string[]).includes(j.status));
    }
    ...
    if (searchVal) {
      list = list.filter((c: Candidate) => c.name!.toLowerCase().includes(searchVal) || c.email!.toLowerCase().includes(searchVal) || c.jobApplied!.toLowerCase().includes(searchVal));
    }
    if (AppState.analyticsCandStageFilter?.length > 0) {
      list = list.filter((c: Candidate) => (AppState.analyticsCandStageFilter as string[]).includes(c.status as string));
    }
    ...
    list.forEach((c: Candidate) => {
      ...
      if (visible.includes('name')) {
        cellsHtml += `
          <td>
            <div class="user-cell">
              <div class="user-avatar-mini">${escapeHTML(c.name!.split(' ').map((n: string) => n[0]).join(''))}</div>
              <div class="user-details">
                <span style="font-weight: 600;">${escapeHTML(c.name)}</span>
                <span class="user-email-mini">${escapeHTML(c.email as string)}</span>
              </div>
            </div>
          </td>
        `;
  ...
  sortHeaders.forEach(th => {
    th.addEventListener('click', () => {
      ...
      if (AppState.jobsSortKey === key) {
        (AppState as AppStateModel).jobsSortAsc = !AppState.jobsSortAsc;
      } else {
        (AppState as AppStateModel).jobsSortKey = key as string;
...
function renderTeamTable() {
  ...
  const searchVal = (document.getElementById('team-search') as HTMLInputElement).value.toLowerCase();
  const roleVal = (document.getElementById('team-role-filter') as HTMLInputElement).value;

  const filteredTeam = (AppState.team as TeamMember[]).filter((member: TeamMember) => {
    // Status filters
  ...
  tbody.querySelectorAll('.team-usertype-select').forEach(sel => {
    sel.addEventListener('change', async (e) => {
      ...
      const member = (AppState.team as TeamMember[]).find((m: TeamMember) => m.email === email);
      if (!member) return;
      ...
      if (isApiMode() && member.backendId) {
        try {
          ...
          (sel as HTMLSelectElement).value = prev as string;
          showPremiumToast(`Could not update ${member.name}'s role: ${(err && (err as Error).message) || 'backend error'}`, 'error');
          return;
        }
      }
    ...
    sel.addEventListener('change', async (e) => {
      ...
      if (isApiMode() && member.backendId) {
        try {
          ...
          (sel as HTMLSelectElement).value = prev as string;
          showPremiumToast(`Could not update ${member.name}'s status: ${(err && (err as Error).message) || 'backend error'}`, 'error');
          return;
        }
      }
      member.status = next;
    ...
    btn.addEventListener('click', async () => {
      ...
      if (isApiMode() && member.backendId) {
        try {
          ...
          showPremiumToast(`Could not revoke ${member.name}: ${(err && (err as Error).message) || 'backend error'}`, 'error');
          return;
        }
      }
      AppState.team = (AppState.team as TeamMember[]).filter((m: TeamMember) => m.email !== email) as any;
      saveStateToLocalStorage();
...
function updateTeamCounters() {
  ...
  document.querySelector('.team-count-all')!.textContent = total as any;
  document.querySelector('.team-count-active')!.textContent = active as any;
  document.querySelector('.team-count-invited')!.textContent = invited as any;
  document.querySelector('.team-count-inactive')!.textContent = inactive as any;
}

// 4. Update Summary Metrics (Analytics View Header Stats)
function parseFuzzyDate(str?: string | null) {
  if (!str) return null;
...
function getDateRangeBounds() {
  ...
  if (AppState.dateRange === 'custom') {
    const from = (document.getElementById('date-from') as HTMLInputElement)?.value || (document.getElementById('jd-date-from') as HTMLInputElement)?.value || (AppState as AppStateModel).customDateFrom;
    const to = (document.getElementById('date-to') as HTMLInputElement)?.value || (document.getElementById('jd-date-to') as HTMLInputElement)?.value || (AppState as AppStateModel).customDateTo;
...
function applyDateRangeGlobally() {
  ...
  const activeJob = (AppState.jobs as Job[]).find((j: Job) => j.id === AppState.activeJobId);
  if (activeJob) {
    const jobCandidates = filterCandidatesByDateRange(
      (AppState.candidates as Candidate[]).filter((c: Candidate) => {
...
function filterCandidatesByDateRange(candidates: Candidate[]): Candidate[] {
  const { start, end } = getDateRangeBounds();
...
function updateSummaryMetrics() {
  ...
  document.getElementById('stat-total-applicants')!.textContent = totalApplicants as any;
  document.getElementById('stat-resume-analysis')!.textContent = resumeCount as any;
  document.getElementById('stat-recruiter-screening')!.textContent = screeningCount as any;
  document.getElementById('stat-functional-interview')!.textContent = functionalCount as any;

  const bySource: Record<string, number> = { 'Career Page': 0, 'Bulk Upload': 0, 'Scheduled': 0, 'Direct Link': 0, 'ATS': 0 };
  filtered.forEach((c: Candidate) => { if (bySource[c.source as string] !== undefined) bySource[c.source as string]++; });

  const appPills = document.querySelectorAll('.card-metric:nth-child(1) .m-pill .v');
  if (appPills.length >= 4) {
    appPills[0].textContent = bySource['Career Page'] as any;
    appPills[1].textContent = bySource['Bulk Upload'] as any;
    appPills[2].textContent = bySource['Scheduled'] as any;
    appPills[3].textContent = bySource['Direct Link'] as any;
  }
  ...
  if (resPills.length >= 3) {
    ...
    resPills[0].textContent = analysed as any;
    resPills[1].textContent = filtered.filter(c => c.status === 'Screening' || c.status === 'Functional').length as any;
    resPills[2].textContent = 0 as any;
  }
  ...
  if (scrPills.length >= 4) {
    ...
    scrPills[0].textContent = attempted as any;
    scrPills[1].textContent = scheduled as any;
    scrPills[2].textContent = 0 as any;
    scrPills[3].textContent = 0 as any;
  }
  ...
  if (funPills.length >= 4) {
    ...
    funPills[0].textContent = attempted as any;
    funPills[1].textContent = scheduled as any;
    funPills[2].textContent = 0 as any;
    funPills[3].textContent = 0 as any;
  }
}
...
function applyUsageStats(s: any) {
  if (!s) return;
  const setText = (id: string, v: any) => { const el = document.getElementById(id); if (el) el.textContent = v ?? 0; };
  ...
  const setPills = (n: number, vals: any[]) => {
    const pills = document.querySelectorAll(`.card-metric:nth-child(${n}) .m-pill .v`);
    vals.forEach((v: any, i: number) => { if (pills[i]) pills[i].textContent = v ?? 0; });
  };
...
async function refreshUsageStats() {
  try {
    const { start, end } = getDateRangeBounds();
    applyUsageStats(await apiFetchUsageStats(start as Date, end as Date));
  } catch (e) {
    showPremiumToast(`Couldn't refresh usage stats: ${(e as any).message || e}`, 'error');
  }
}
...
async function hydrateUsageAnalytics() {
  ...
  try {
    ...
    const [stats, candidates] = await Promise.all([
      apiFetchUsageStats(start as Date, end as Date),
      apiFetchUsageCandidates(),
    ]);
    candidates.forEach((c: Candidate) => {
      const job = (AppState.jobs as Job[]).find((j: Job) => j.id === c.jobId);
      c.jobApplied = job ? (job.roleName || job.cardName || '') : (c.jobApplied || '');
    });
    AppState.candidates = candidates as any;
    applyUsageStats(stats);
    renderAnalyticsTable();
  } catch (e) {
    showPremiumToast(`Couldn't load usage data: ${(e as any).message || e}`, 'error');
  }
}
