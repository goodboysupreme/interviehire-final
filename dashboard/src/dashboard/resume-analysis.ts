...
import type { Candidate, Job, ResumeAnalysis, ResumeCriteria, ScoringConfig } from '../types/models';

// ==========================================
// RESUME ANALYSIS (AI-powered, Lina)
// ==========================================

const resumeTextCache: Record<string, string | null> = {};
const resumeIdentityCache: Record<string, any> = {};
const resumeAnalysisCache: Record<string, any> = {};
const reportChatCache: Record<string, any> = {};

// The resume-stage analysis operates on fully-populated criteria/config objects
// (defaults fill the gaps), so use concrete shapes locally to keep array access typed.
type Criteria = { mustHave: string[]; redFlags: string[]; goodToHave: string[]; goodToHaveMinMatch?: number };
type ScoringCfg = ScoringConfig & { customCriteria: Array<{ label: string; weight: number; description?: string }>; mustHaveGate?: boolean; mustHaveCap: number };
type AnalysisResult = ResumeAnalysis & Record<string, any>;
...
function cacheResumeTextAndIdentity(cid: string, text: string, filename = '') {
  if (!text || isGarbageText(text)) return null;

  resumeTextCache[cid] = text;
  const candidate = AppState.candidates.find((c: Candidate) => c.id === cid) as (Candidate & Record<string, any>) | undefined;
...
function refreshResumeCandidateRowIdentity(cid: string) {
  const candidate = AppState.candidates.find((c: Candidate) => c.id === cid) as Candidate | undefined;
  ...
  if (nameEl) nameEl.textContent = candidate.name as string;
...
function renderResumeStagePaneForJob(candidates: Candidate[], job: Job, container: HTMLElement) {
  // Hydrate the in-memory cache from analyses persisted on candidates
  candidates.forEach((c: Candidate) => {
    if (!resumeAnalysisCache[c.id as string] && c.resumeAnalysis) resumeAnalysisCache[c.id as string] = c.resumeAnalysis;
    if (!resumeTextCache[c.id as string] && c.resumeText) resumeTextCache[c.id as string] = c.resumeText;
  });
  ...
  const pendingCount = candidates.filter((c: Candidate) => !resumeAnalysisCache[c.id as string]).length;
...
function bindResumeAnalysisEvents(job: Job) {
  document.querySelectorAll('.ra-data-table tr[data-cid]').forEach(row => {
    const cid = (row as HTMLElement).dataset.cid as string;
    const fileInput = document.getElementById(`ra-file-${cid}`) as HTMLInputElement | null;
    ...
    const pasteArea = document.getElementById(`ra-paste-${cid}`) as HTMLTextAreaElement | null;
  ...
  analyseAllBtn?.addEventListener('click', () => {
    ...
    document.querySelectorAll('.ra-data-table tr[data-cid]').forEach(row => {
      const rcid = (row as HTMLElement).dataset.cid as string;
      if (!resumeAnalysisCache[rcid]) {
        pendingCids.push(rcid);
      }
    });
  ...
  const importInput = document.getElementById('ra-import-file') as HTMLInputElement | null;
  ...
  reanalyseAllBtn?.addEventListener('click', () => {
    const analysedCids: string[] = [];
    document.querySelectorAll('.ra-data-table tr[data-cid]').forEach(row => {
      const rcid = (row as HTMLElement).dataset.cid as string;
      if (resumeAnalysisCache[rcid]) analysedCids.push(rcid);
    });
...
function extractNameFromResumeText(text: string) {
  return extractResumeIdentity(text).name || null;
}
async function handleResumeFile(cid: string, file: File) {
...
function extractExperienceYearsFromText(text: string) {
...
function asStringArray(v: any, max = 12) {
  if (!Array.isArray(v)) return [];
  return v.filter(Boolean).map(x => String(x).trim()).filter(Boolean).slice(0, max);
}

function buildCriteriaBlock(criteria: Criteria, config: ScoringCfg) {
  const lines: string[] = [];
  ...
  if (config.customCriteria.length) {
    ...
    config.customCriteria.forEach((c, i: number) => {
...
function deriveLegacyScorecard(dims: any) {
  return {
...
function applyGatesAndScore(result: AnalysisResult, config: ScoringCfg, criteria: Criteria) {
  ...
  const missingMust = (result.criteriaVerdicts || [])
    .filter((v: any) => v.group === 'mustHave' && v.met !== true && v.met !== 'true' && v.met !== 'partial')
    .map((v: any) => v.criterion);
...
function normalizeDeepResult(result: AnalysisResult, config: ScoringCfg, criteria: Criteria) {
  const dims: Record<string, any> = result.dimensions && typeof result.dimensions === 'object' ? result.dimensions : {};
  ...
  result.projects = (Array.isArray(result.projects) ? result.projects : []).slice(0, 6).map((p: any) => ({
  ...
  result.competencies = (Array.isArray(result.competencies) ? result.competencies : []).slice(0, 8).map((c: any) => ({
    name: String(c?.name || 'Competency'),
    score: clampScore(c?.score),
    bullets: asStringArray(c?.bullets, 5),
  }));

  result.criteriaVerdicts = (Array.isArray(result.criteriaVerdicts) ? result.criteriaVerdicts : []).slice(0, 30).map((v: any) => ({
...
function relevanceOverlap(textTokens: Set<string>, jdTokens: Set<string>) {
  let hits = 0, total = 0;
  jdTokens.forEach((t: string) => {
...
function extractProjectsLocally(resumeText: string, job: Job, criteria: Criteria) {
  const jdTokens = tokenize(`${job.roleName} ${job.description || ''} ${criteria.mustHave.join(' ')} ${criteria.goodToHave.join(' ')}`);
  const lines = resumeText.split('\n');
  const projects: Array<{ name: string; body: string }> = [];
...
function matchCriterion(resumeLower: string, criterion: string): boolean | 'partial' {
  const clean = criterion.replace(/[^\w\s+#.]/g, '').toLowerCase().trim();
  if (!clean) return false;
  if (resumeLower.includes(clean)) return true;
  const words = clean.split(/\s+/).filter((w: string) => w.length > 3 && !STOPWORDS.has(w));
  if (!words.length) return false;
  const hits = words.filter((w: string) => resumeLower.includes(w)).length;
  return hits === words.length ? true : hits / words.length >= 0.5 ? 'partial' : false;
}

function buildLocalDeepAnalysis(resumeText: string, job: Job, config: ScoringCfg, criteria: Criteria) {
  const resumeLower = resumeText.toLowerCase();
  const verdicts: Array<{ criterion: string; group: string; met: boolean | 'partial'; evidence: string }> = [];
  const matched: string[] = [], missing: string[] = [];

  criteria.mustHave.forEach((c: string) => {
  ...
  criteria.goodToHave.forEach((c: string) => {
    const met = matchCriterion(resumeLower, c);
  ...
  const customWeightSum = customScores.reduce((s: number, c) => s + c.weight, 0);
  const customScore = customWeightSum ? Math.round(customScores.reduce((s: number, c) => s + c.score * c.weight, 0) / customWeightSum) : 0;

  const redFlagsFound = criteria.redFlags.filter((f: string) => matchCriterion(resumeLower, f) === true);
  ...
  const detected = ['JavaScript', 'TypeScript', 'React', 'Node.js', 'Python', 'SQL', 'AWS', 'Docker', 'Excel', 'Project Management', 'Agile', 'Communication', 'Proposal Writing', 'Compliance', 'Tender Management']
    .filter((s: string) => resumeLower.includes(s.toLowerCase())).slice(0, 8);

  const projects = extractProjectsLocally(resumeText, job, criteria);
  const projectsScore = projects.length ? Math.round(projects.reduce((s: number, p) => s + p.relevance, 0) / projects.length) : 30;
...
async function runResumeDeepAnalysis(resumeText: string, job: Job, criteria: Criteria, criteriaBlock: string) {
...
async function ensureResumeText(cid: string, candidate: (Candidate & { resumeLink?: string | null }) | undefined) {
  let text = ((resumeTextCache[cid] || '') + '\n' + ((document.getElementById(`ra-paste-${cid}`) as HTMLTextAreaElement | null)?.value || '')).trim();
...
async function prefetchIdentities(cids: string[]) {
  let cursor = 0;
  const worker = async () => {
...
async function runResumeAnalysis(cid: string, job: Job, opts: { quiet?: boolean; force?: boolean } = {}) {
  const quiet = opts.quiet === true;
  const btn = document.getElementById(`ra-btn-${cid}`) as HTMLButtonElement | null;
  ...
  const criteria = (job.resumeCriteria || { mustHave: [], redFlags: [], goodToHave: [] }) as Criteria;
  const config = getScoringConfig(job) as ScoringCfg;
  ...
  const cand = AppState.candidates.find((c: Candidate) => c.id === cid) as Candidate | undefined;
  if (cand) {
    ...
    if (cand._backend && getDataSource() === 'api') {
      ...
      if (!cand.email) {
        ...
        const registered = cand.backendId
          ...
          : apiAddApplicant(job.id as string, { name: cand.name, email: cand.email, phone: cand.phone, entryMethod: cand.entryMethod }).then((created: any) => {
              if (!created || !created.id) throw new Error('Could not register the candidate in the backend.');
              cand.backendId = created.id;
              cand.jobId = job.id as string;
              saveStateToLocalStorage();
              return created.id;
            });
...
function renderAnalysisResult(cid: string, result: AnalysisResult) {
  const row = document.querySelector(`tr[data-cid="${cid}"]`);
  if (!row) return;
...
async function runBulkResumeAnalysis(candidateIds: string[], job: Job, opts: { force?: boolean } = {}) {
  const force = opts.force === true;
  // Normal bulk skips already-analysed; reanalyse (force) re-runs them against the new params.
  const pending = force ? candidateIds.slice() : candidateIds.filter((id: string) => !resumeAnalysisCache[id]);
  if (pending.length === 0) {
    showPremiumToast('All candidates already analysed.', 'info');
    return;
  }
...
function toggleResumeCriteriaEdit(job: Job) {
  ...
  if (isEditing) {
    ...
    const criteria: Criteria & { goodToHaveMinMatch: number } = { mustHave: [], redFlags: [], goodToHave: [], goodToHaveMinMatch: 1 };
    section.querySelectorAll('.ra-criteria-group.must-have .ra-criteria-edit-input').forEach(input => {
      if ((input as HTMLInputElement).value.trim()) criteria.mustHave.push((input as HTMLInputElement).value.trim());
    });
    section.querySelectorAll('.ra-criteria-group.red-flags .ra-criteria-edit-input').forEach(input => {
      if ((input as HTMLInputElement).value.trim()) criteria.redFlags.push((input as HTMLInputElement).value.trim());
    });
    section.querySelectorAll('.ra-criteria-group.good-to-have .ra-criteria-edit-input').forEach(input => {
      if ((input as HTMLInputElement).value.trim()) criteria.goodToHave.push((input as HTMLInputElement).value.trim());
    });
    const minMatch = section.querySelector('.ra-min-match-input');
    if (minMatch) criteria.goodToHaveMinMatch = parseInt((minMatch as HTMLInputElement).value) || 1;
    ...
    if (resumeList) {
      ...
      const resumeCands = jobCandidates.filter((c: Candidate) => c.status === 'Resume');
  ...
  const criteria: Criteria & { goodToHaveMinMatch?: number } = (job.resumeCriteria as Criteria) || { mustHave: [], redFlags: [], goodToHave: [], goodToHaveMinMatch: 1 };
  ...
  section.querySelectorAll('.ra-criteria-items').forEach(itemsContainer => {
    ...
    itemsContainer.innerHTML = items.map((item: string, i: number) => `
    ...
    itemsContainer.querySelectorAll('.btn-ra-remove-criteria').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        itemsContainer.querySelectorAll('.ra-criteria-num').forEach((num, idx) => {
          num.textContent = (idx + 1) as unknown as string;
        });
      });
    });
    ...
    itemsContainer.querySelector('.btn-ra-add-criteria')?.addEventListener('click', () => {
      ...
      newItem.querySelector('.btn-ra-remove-criteria')!.addEventListener('click', () => {
        newItem.remove();
        itemsContainer.querySelectorAll('.ra-criteria-num').forEach((num, idx) => { num.textContent = (idx + 1) as unknown as string; });
      });
      (newItem.querySelector('input') as HTMLInputElement).focus();
    });
  });
...
function matrixToCandidates(matrix: string[][]) {
  if (!matrix.length) return [];
  const headers = matrix[0].map((h: string) => String(h).toLowerCase().trim());
  const exact = (...keys: string[]) => headers.findIndex((h: string) => keys.includes(h));
  const contains = (...subs: string[]) => headers.findIndex((h: string) => subs.some(s => h.includes(s)));
...
async function handleResumeImportFile(file: File, job: Job) {
  let matrix: string[][] = [];
  ...
  try {
    if (fname.endsWith('.xlsx') || fname.endsWith('.xls')) {
      ...
      matrix = (XLSX.utils.sheet_to_json(sheet, { header: 1, blankrows: false, defval: '' }) as any[])
  ...
  for (const r of rows) {
    try {
      const cid = await (addCandidateToAppState as any)(r.name || r.email || 'Imported Candidate', r.email, r.phone, job);
      const cand = AppState.candidates.find((c: Candidate) => c.id === cid) as (Candidate & { resumeLink?: string | null }) | undefined;
...
function formatSlot(dtLocal: string) {
  if (!dtLocal) return '';
...
function createDateTimePicker(initial: Date) {
  ...
  field.addEventListener('click', (e) => {
    ...
    el.closest('.schedule-modal')?.querySelectorAll('.sdt-pop').forEach(p => { (p as HTMLElement).hidden = true; });
    if (!open) { renderCal(); pop.hidden = false; }
  });
  pop.addEventListener('click', (e) => {
    e.stopPropagation();
    const target = e.target as HTMLElement;
    const nav = target.closest('.sdt-nav') as HTMLElement | null;
    if (nav) { view.setMonth(view.getMonth() + Number(nav.dataset.nav)); renderCal(); return; }
    const day = target.closest('.sdt-day') as HTMLElement | null;
  ...
  pop.addEventListener('change', (e) => {
    const target = e.target as HTMLInputElement;
    if (target.classList.contains('sdt-time')) {
      const [h, mi] = target.value.split(':').map(Number);
...
function openScheduleModal(opts: any, callback?: any) {
  ...
  const ro = (label: string, value: string) => `<div class="sched-readonly-row"><span class="sched-ro-label">${label}</span><span class="sched-ro-value">${value}</span></div>`;
  ...
  modalEl.addEventListener('click', (e) => {
    if (!(e.target as HTMLElement).closest('.sdt')) modalEl.querySelectorAll('.sdt-pop').forEach(p => { (p as HTMLElement).hidden = true; });
  });
...
function sfdIcon(value: string) {
  const paths = (SFD_ICONS as Record<string, string>)[value];
...
function buildFilterDropdown(chip: any, type: string, candidates: Candidate[], stageKey: string) {
  ...
  const filters = (AppState.stageFilters as any)[stageKey] as any;

  if (type === 'interviewStatus') {
    const statuses = ['Completed', 'Incomplete', 'Evaluating', 'Attempting', 'Not Started', 'Slot Missed'];
    const counts: Record<string, number> = {};
    statuses.forEach(s => { counts[s] = candidates.filter((c: Candidate) => c.interviewStatus === s).length; });
    dd.innerHTML = `
    ...
    dd.querySelectorAll('input[type=checkbox]').forEach(cb => cb.addEventListener('change', () => {
      filters.interviewStatus = [...dd.querySelectorAll('input[type=checkbox]:checked')].map(c => (c as HTMLInputElement).value);
      const activeJob = AppState.jobs.find((j: Job) => j.id === AppState.activeJobId);
      if (activeJob) renderJobDetailPanes(activeJob);
    }));
    dd.querySelector('.sfd-clear-btn')!.addEventListener('click', () => { filters.interviewStatus = []; const activeJob = AppState.jobs.find((j: Job) => j.id === AppState.activeJobId); if (activeJob) renderJobDetailPanes(activeJob); });
  } else if (type === 'cheatProb') {
    const levels = ['High', 'Medium', 'Low'];
    const counts: Record<string, number> = {};
    levels.forEach(l => { counts[l] = candidates.filter((c: Candidate) => c.cheatProbability === l).length; });
    dd.innerHTML = `
      ...
      <div class="sfd-footer"><button class="sfd-clear-btn">Clear filters</button></div>`;
    dd.querySelectorAll('input[type=checkbox]').forEach(cb => cb.addEventListener('change', () => {
      filters.cheatProb = [...dd.querySelectorAll('input[type=checkbox]:checked')].map(c => (c as HTMLInputElement).value);
      const activeJob = AppState.jobs.find((j: Job) => j.id === AppState.activeJobId); if (activeJob) renderJobDetailPanes(activeJob);
    }));
    dd.querySelector('.sfd-clear-btn')!.addEventListener('click', () => { filters.cheatProb = []; const activeJob = AppState.jobs.find((j: Job) => j.id === AppState.activeJobId); if (activeJob) renderJobDetailPanes(activeJob); });
  } else if (type === 'recruiterScreening') {
    const vals = ['Good fit', 'Moderate fit', 'Poor fit'];
    const counts: Record<string, number> = {};
    vals.forEach(v => { counts[v] = candidates.filter((c: Candidate) => c.recruiterScreening === v).length; });
    dd.innerHTML = `
      ...
      <div class="sfd-footer"><button class="sfd-clear-btn">Clear filters</button></div>`;
    dd.querySelectorAll('input[type=checkbox]').forEach(cb => cb.addEventListener('change', () => {
      filters.recruiterScreening = [...dd.querySelectorAll('input[type=checkbox]:checked')].map(c => (c as HTMLInputElement).value);
      const activeJob = AppState.jobs.find((j: Job) => j.id === AppState.activeJobId); if (activeJob) renderJobDetailPanes(activeJob);
    }));
    dd.querySelector('.sfd-clear-btn')!.addEventListener('click', () => { filters.recruiterScreening = []; const activeJob = AppState.jobs.find((j: Job) => j.id === AppState.activeJobId); if (activeJob) renderJobDetailPanes(activeJob); });
    ...
    dd.querySelector('.sfd-btn-apply')!.addEventListener('click', () => {
      filters.scoreMin = parseInt((dd.querySelector('#sfd-score-min') as HTMLInputElement).value) || 0;
      filters.scoreMax = parseInt((dd.querySelector('#sfd-score-max') as HTMLInputElement).value) || 100;
      const activeJob = AppState.jobs.find((j: Job) => j.id === AppState.activeJobId); if (activeJob) renderJobDetailPanes(activeJob);
    });
    dd.querySelector('.sfd-btn-clear')!.addEventListener('click', () => { filters.scoreMin = null; filters.scoreMax = null; const activeJob = AppState.jobs.find((j: Job) => j.id === AppState.activeJobId); if (activeJob) renderJobDetailPanes(activeJob); });
...
function applyStageFilters(candidates: Candidate[], stageKey: string) {
  const f = (AppState.stageFilters as any)[stageKey] as any;
  if (!f) return candidates;
  let filtered = candidates;
  if (f.interviewStatus.length > 0) filtered = filtered.filter((c: Candidate) => f.interviewStatus.includes(c.interviewStatus));
  if (f.cheatProb.length > 0) filtered = filtered.filter((c: Candidate) => f.cheatProb.includes(c.cheatProbability));
  if (f.recruiterScreening.length > 0) filtered = filtered.filter((c: Candidate) => f.recruiterScreening.includes(c.recruiterScreening));
  if (f.scoreMin != null) filtered = filtered.filter((c: Candidate) => c.interviewScore != null && c.interviewScore >= f.scoreMin);
  if (f.scoreMax != null) filtered = filtered.filter((c: Candidate) => c.interviewScore != null && c.interviewScore <= f.scoreMax);
  return filtered;
}

function hasActiveFilters(stageKey: string) {
  const f = (AppState.stageFilters as any)[stageKey] as any;
