...
class SoundEngine {
  ...
  playSlider(value: number, min = 0, max = 100) {
    if (this.muted || !this.ctx) return;
...
export function initLandingPage() {
  ...
  cards.forEach((card: Element) => {
    ...
    cardEl.addEventListener('mousemove', (e: MouseEvent) => {
      ...
      cardEl.style.transform = `perspective(1000px) rotateX(${angleX}deg) rotateY(${angleY}deg)`;
    }, { signal });
  ...
  const video = document.getElementById('pipeline-video') as HTMLVideoElement | null;
  const statusText = document.getElementById('video-status-text');
  const statusDot = document.querySelector('.video-status-badge .status-dot') as HTMLElement | null;

  interface VideoRange { min: number; max: number; label: string; color: string; }
  const videoRanges: Record<string, VideoRange> = {
  ...
  function updatePipeline(tabId: string) {
    ...
    if (video) {
      ...
      if (video.currentTime < range.min || video.currentTime > range.max) {
        ...
        video.play().catch((err: any) => console.log("Play failed on range switch:", err));
      }
    }
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      updatePipeline(targetTab as string);
    }, { signal });
  });
  ...
  const hiresSlider = document.getElementById('hires-slider') as HTMLInputElement | null;
  const salarySlider = document.getElementById('salary-slider') as HTMLInputElement | null;
  ...
  function calculateSavings() {
    ...
    valHiresDisplay!.textContent = hires as any;
  ...
  if (hiresSlider && salarySlider) {
    hiresSlider.addEventListener('input', (e) => {
      calculateSavings();
      soundEngine.playSlider(parseFloat((e.target as HTMLInputElement).value), parseFloat(hiresSlider.min), parseFloat(hiresSlider.max));
    }, { signal });
    salarySlider.addEventListener('input', (e) => {
      calculateSavings();
      soundEngine.playSlider(parseFloat((e.target as HTMLInputElement).value), parseFloat(salarySlider.min), parseFloat(salarySlider.max));
    }, { signal });
    calculateSavings();
  }
  ...
  const pilotForm = document.getElementById('pilot-form') as HTMLFormElement | null;
  const formSuccess = document.getElementById('form-success');

  if (pilotForm) {
    pilotForm.addEventListener('submit', (event) => {
      event.preventDefault();
      const submitBtn = pilotForm.querySelector('button[type="submit"]') as HTMLButtonElement | null;
      if (submitBtn) {
  ...
  if (document.querySelector('.transition-morph-section')) {
    ...
    const playScanSweep = (freqProgress: number) => {
