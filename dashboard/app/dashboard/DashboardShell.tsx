...
import { apiGetPreferences, apiUpdatePreferences } from '../../src/dashboard/api';
...
function navigateToPath(path: string | null) {
  ...
  if (!sub) {
    (window as any).navigateToTab?.('jobs');
    return;
  }
  ...
  if (sub === 'jobs') {
    ...
    if (jobId) {
      if (subSub === 'flow') {
        (window as any).openJobFlowView?.(jobId);
      } else {
        (window as any).navigateToJobStage?.(jobId, subSub || 'overview');
      }
    } else {
      (window as any).navigateToTab?.('jobs');
    }
    ...
    const jobId = rawJobId && rawJobId.includes('--') ? rawJobId.split('--').pop() : rawJobId;
    if (jobId) {
      (window as any).navigateToSourcing?.(jobId);
    }
  } else if (sub === 'settings') {
    (window as any).navigateToSubtab?.('settings-general');
  } else if (['analytics', 'swarm', 'talent', 'team', 'career'].includes(sub)) {
    (window as any).navigateToTab?.(sub);
  }
}
...
export default function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const [phase, setPhase] = useState<'checking' | 'authed'>('checking');
  ...
  useEffect(() => {
    (window as any).__ihPushState = (url: string) => {
      router.push(url, { scroll: false });
    };
    (window as any).__ihNavigateToPath = (path: string | null) => {
      navigateToPath(path);
    };
    return () => {
      delete (window as any).__ihPushState;
      delete (window as any).__ihNavigateToPath;
    };
  }, [router]);
  ...
  useEffect(() => {
    (window as any).IH_updateTheme = async (theme: string) => {
      await apiUpdatePreferences({ theme });
    };
    return () => { delete (window as any).IH_updateTheme; };
  }, []);
  ...
  useEffect(() => {
    ...
    if (roleEl) roleEl.textContent = (ROLE_LABEL as any)[user.user_type] || 'Member';
    ...
    const creatorInput = document.getElementById('job-creator-input') as HTMLInputElement | null;
  ...
  useEffect(() => {
    ...
    let timer: any;
    const bind = () => {
      const btn = document.querySelector('.user-profile .btn-logout') as HTMLElement | null;
